...
function renderStatuses(){ $('statusList').innerHTML=statuses.map(s=>`<div class="status-row"><span class="swatch" style="background:${safeColor(s.color)}"></span><input class="sname" data-id="${s.id}" value="${escAttr(s.name)}"><input class="scolor" data-id="${s.id}" type="color" value="${safeColor(s.color)}"><button class="small-btn" data-edit="${s.id}">Save</button><button class="small-btn danger" data-delete="${s.id}">Delete</button></div>`).join('') }
